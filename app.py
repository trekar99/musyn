...
with gr.Blocks(title = config.title, css = config.css) as demo:
    ...
    with gr.Tab("Live Audio"):
      ...
      input_audio.stream(captioning, 
                        [input_audio, input_caption], 
                        [output_cpt, output_img, output_audio], 
                        time_limit=0, stream_every=0.05, concurrency_limit=30
                        )
      #seed.change(generator.image_generator, [input_caption, seed, output_cpt], output_img, show_progress="minimal")
    ...
    with gr.Tab("File Audio"):
      ...
      with gr.Row():
        btn = gr.Button("Run")
        btn.click(fn=captioning, inputs=[input_audio, input_caption], outputs=[output_cpt, output_img, output_audio])
